...
public class BanAnDAO {
    ...
    public String LayTinhTrangBanTheoMa(int maban){
        String tinhtrang = "";
        String truyvan = "select * from " + CreateDatabase.TB_BANAN + " where " + CreateDatabase.TB_BANAN_MABAN + " = '" +
                maban + "'";
        Cursor cursor = database.rawQuery(truyvan,null);
        cursor.moveToFirst();
        while (!cursor.isAfterLast()){
    ...
    public boolean CapNhatLaiTinhTrangBan(int maban, String tinhtrang){
        ...
        long kiemtra  = database.update(CreateDatabase.TB_BANAN,contentValues,
    ...
    public boolean XoaBanAnTheoMa(int maban){
        ...
        long kiemtra = database.delete(CreateDatabase.TB_BANAN,CreateDatabase.TB_BANAN_MABAN + " = " +
                maban,null);
        if (kiemtra!=0){
            return  true;
        }else {
